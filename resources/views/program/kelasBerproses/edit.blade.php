...
@section('content')
    ...
    <div class="flex justify-center item-center mt-8">
        <h1 class="text-2xl font-bold">Form Edit Data Kelas Berproses</h1>
    </div>
    <div class="flex justify-center item-center">
        <form method="POST" action="/program/kelas-berproses/{{ $kelasberproses->id }}" class="w-6/12" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="bg-abu rounded-lg">
        <div class="m-10 py-10">
        <div class="flex flex-col md:flex-row pb-4 mb-4">
                <div class="w-44 font-bold h-6 mx-2 mt-3">Pilihan Kelas Berproses</div>
                <div>
                    <div class="flex flex-col md:flex-row w-max">
                        <div class="flex-1 my-2 p-1 bg-white flex border border-gray-200 rounded-lg">
                            <input type="text" class="p-1 px-2 w-96" name="pilihan_kelasberproses" value="{{ old('pilihan_kelasberproses', $kelasberproses->pilihan_kelasberproses) }}">
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex flex-col md:flex-row pb-4 mb-4">
                <div class="w-44 font-bold h-6 mx-2 mt-3">Bukti Transfer</div>
                <div>
                    <div class="flex flex-col md:flex-row w-max">
                        <div class="flex-1 my-2 p-1 bg-white flex border border-gray-200 rounded-lg">
                            <input type="text" class="p-1 px-2 py-10 w-96" name="bukti_transfer" value="{{ old('bukti_transfer', $kelasberproses->bukti_transfer) }}">
                        </div>
                    </div>
                </div>
            </div>
            <input type="hidden" class="p-1 px-2 w-full" name="user_id" value="{{ session()->get('id') }}">
        </div>
        </div>
