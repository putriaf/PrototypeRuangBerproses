...
<div class="ml-16 mr-20 max-w-3xl flex-auto">
    <div class="">
        <span class="flex max-w-44">
            <h1 class="text-4xl font-extrabold text-dongker line-clamp-2">{{ $kelasberproses->pilihan_kelasberproses }}</h1>
            @guest
            @else
            @if($kelasberproses->user_id == session()->get('id'))
        ...
        <form action="/program/kelas-berproses/{{ $kelasberproses->id }}" method="post">
            @method('delete')
            @csrf
    ...
    <div class="flex items-start px-2 py-6">
        ...
        <div class="pt-1">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-gray-900 mt-1">{{ $kelasberproses->nama }}</h2>
            </div>
            <p class="text-gray-700">{{ $kelasberproses->created_at }}2</p>
        </div>
    </div>

    <div class="mt-5 text-justify">
        <p>{{ $kelasberproses->id }}</p>
    </div>
    @guest
    @else
    @if($psytalk->user_id == session()->get('id'))
    @else
