...
<div class="ml-16 mr-20 max-w-3xl flex-auto">
    <div class="">
        <span class="flex max-w-44">
            <h1 class="text-4xl font-extrabold text-dongker line-clamp-2">{{ $psytalk->pilihan_webinar }}</h1>
            @guest
            @else
        ...
        <form action="/program/psytalk/{{ $psytalk->id }}" method="post">
            @method('delete')
            @csrf
    ...
    <div class="flex items-start px-2 py-6">
        ...
        <div class="pt-1">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-gray-900 mt-1">{{ $psytalk->nama }}</h2>
            </div>
            <p class="text-gray-700">{{ $psytalk->created_at }}2</p>
        </div>
    </div>
