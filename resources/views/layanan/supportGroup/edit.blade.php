...
@section('content')
    ...
    <div class="flex justify-center item-center mt-8">
        <h1 class="text-2xl font-bold">Form Edit Data Support Group</h1>
    </div>
    <div class="flex justify-center item-center">
        <form method="POST" action="/layanan/support-group/{{ $supportgroup->id }}" class="w-6/12" enctype="multipart/form-data">
        @method('put')
        @csrf
                ...
                <div>
                    <div class="flex flex-col md:flex-row w-max">
                        <div class="flex-1 my-2 p-1 bg-white flex border border-gray-200 rounded-lg">
                            <input type="text" class="p-1 px-2 w-96" name="pengalaman" value="{{ old('pengalaman', $supportgroup->pengalaman) }}">
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex flex-col md:flex-row pb-4 mb-4">
                <div class="w-44 font-bold h-6 mx-2 mt-3">Fasilitator</div>
                <div>
                    <div class="flex flex-col md:flex-row w-max">
                        <div class="flex-1 my-2 p-1 bg-white flex border border-gray-200 rounded-lg">
                            <input type="text" class="p-1 px-2 py-10 w-96" name="fasilitator" value="{{ old('fasilitator', $supportgroup->fasilitator) }}">
                        </div>
                    </div>
                </div>
            </div>
