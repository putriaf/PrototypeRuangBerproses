...
<div class="ml-16 mr-20 max-w-3xl flex-auto">
    <div class="">
        <span class="flex max-w-44">
            <h1 class="text-4xl font-extrabold text-dongker line-clamp-2">{{ $supportgroup->pengalaman }}</h1>
            @guest
            @else
            @if($supportgroup->user_id == session()->get('id'))
        ...
        <form action="/layanan/support-group/{{ $supportgroup->id }}" method="post">
            @method('delete')
            @csrf
        ...
        <div id="dropdown" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700">
            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefault">
                ...
                <li>
                    <form action="/layanan/supportgroup/{{ $supportgroup->id }}" method="post">
                        @method('delete')
                        @csrf
    ...
    <div class="flex items-start px-2 py-6">
        ...
        <div class="pt-1">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-gray-900 mt-1">{{ $supportgroup->nama }}</h2>
            </div>
            <p class="text-gray-700">{{ $supportgroup->created_at }}2</p>
        </div>
    </div>
