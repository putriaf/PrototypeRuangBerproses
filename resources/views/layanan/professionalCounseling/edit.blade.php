...
@section('content')
    ...
    <div class="flex justify-center item-center mt-8">
        <h1 class="text-2xl font-bold">Form Edit Data Professional Counseling</h1>
    </div>
    <div class="flex justify-center item-center">
        <form method="POST" action="/layanan/professional-counseling/{{ $professionalcounseling->id }}" class="w-6/12" enctype="multipart/form-data">
        @method('put')
        @csrf
        ...
        <div class="flex flex-col md:flex-row pb-4 mb-4">
                ...
                <div>
                    <div class="flex flex-col md:flex-row w-max">
                        <div class="flex-1 my-2 p-1 bg-white flex border border-gray-200 rounded-lg">
                            <input type="text" class="p-1 px-2 w-96" name="status" value="{{ old('status', $professionalcounseling->status) }}">
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex flex-col md:flex-row pb-4 mb-4">
                ...
                <div>
                    <div class="flex flex-col md:flex-row w-max">
                        <div class="flex-1 my-2 p-1 bg-white flex border border-gray-200 rounded-lg">
                            <input type="text" class="p-1 px-2 py-10 w-96" name="bukti_transfer" value="{{ old('bukti_transfer', $professionalcounseling->bukti_transfer) }}">
                        </div>
                    </div>
                </div>
            </div>
