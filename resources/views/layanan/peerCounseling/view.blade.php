...
<div class="ml-16 mr-20 max-w-3xl flex-auto">
    <div class="">
        <span class="flex max-w-44">
            <h1 class="text-4xl font-extrabold text-dongker line-clamp-2">{{ $peercounseling->pengalaman }}</h1>
            @guest
            @else
            @if($peercounseling->user_id == session()->get('id'))
        ...
        <form action="/layanan/peer-counseling/{{ $peercounseling->id }}" method="post">
            @method('delete')
            @csrf
            <button type="submit" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" onclick="return confirm('Apakah Anda yakin ingin menghapus post?')">Hapus</button>
        </form>
        </span>
        <!-- Dropdown menu -->
        @guest
        ...
        <div id="dropdown" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700">
            <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefault">
                ...
                <li>
                    <form action="/layanan/peer-counseling/{{ $supportgroup->id }}" method="post">
                        @method('delete')
                        @csrf
    ...
    <div class="flex items-start px-2 py-6">
        ...
        <div class="pt-1">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-gray-900 mt-1">{{ $peercounseling->nama }}</h2>
            </div>
            <p class="text-gray-700">{{ $peercounseling->created_at }}2</p>
        </div>
    </div>

    <div class="mt-5 text-justify">
        <p>{{ $peercounseling->id }}</p>
    </div>
    @guest
    @else
    @if($peercounseling->user_id == session()->get('id'))
